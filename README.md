...
(Note that in cumulative_flux.m, we actually solve for $\hat{u}/s$, which is computationally more stable and yields the cumulative flux directly as opposed to the flux at a given time, which can be recovered by differentiation.)
